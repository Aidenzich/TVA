...
class NCELoss(nn.Module):
    ...
    def forward(self, batch_sample_one, batch_sample_two):  # batch_size*
        ...
        sim11 = (
            self.cossim(batch_sample_one.unsqueeze(-2), batch_sample_one.unsqueeze(-3))
            / self.temperature
        )
        sim22 = (
            self.cossim(batch_sample_two.unsqueeze(-2), batch_sample_two.unsqueeze(-3))
            / self.temperature
        )
        sim12 = (
            self.cossim(batch_sample_one.unsqueeze(-2), batch_sample_two.unsqueeze(-3))
            / self.temperature
        )
...
class VariationalDropout(nn.Module):
    def __init__(self, inputshape, p=0.2, adaptive=None):
        ...
        if self.adaptive == None:
            self.logitalpha = nn.Parameter(
                torch.tensor(_logit(np.sqrt(p / (1.0 - p)))), requires_grad=False
            )

        elif self.adaptive == "layerwise":
            self.logitalpha = nn.Parameter(
                torch.tensor(_logit(np.sqrt(p / (1.0 - p)))), requires_grad=True
            )
            ...
            self.logitalpha = nn.Parameter(
                torch.tensor(
                    np.ones(inputshape[1:]).astype(np.float32)
                    * _logit(np.sqrt(p / (1.0 - p)))
                ),
                requires_grad=True,
            )
...
def priorKL(alpha):
    ...
    return -torch.mean(
        0.5 * torch.log(alpha)
        + c1 * alpha
        + c2 * torch.pow(alpha, 2)
        + c3 * torch.pow(alpha, 3)
    )
