...
def tuner(model_params, trainer_config, dataclass, model_name):
...
def __trainable_function__(config):
...
def __get_tuneconfig__(model_params, trainer_config, dataclass, model_name):
    config = {
...
def __rollback_tuneconfig__(config):
...
def __list2tunechoice__(config):
