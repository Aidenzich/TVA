...
class Charter:
    ...
    def with_rangeslider(self, fig: plotly.graph_objs._figure.Figure) -> plotly.graph_objs._figure.Figure:
    ...
    def transaction_timeseries_plotly(self) -> plotly.graph_objects.Figure:
        ...
        fig = self.with_rangeslider(fig)
        fig = self.with_rangeselector(fig)
        return fig
    ...
    def transactions_per_interval_plotly(
        ...
        data = self.df[self.DATE_COL].dt.to_period(interval) \
    ...
    def num_of_transactions_per_interval(
        ...
        **kwargs,
        ...
        plot_data = self.df[self.DATE_COL].dt.to_period(interval).value_counts().sort_index()
        ...
        if draw_mean_std:
            ...
            _ = ax.text(x=0, y=y.mean() + y.std(), s=f'+std: {y.mean() + y.std():.2f}', color='r')
            _ = ax.text(x=0, y=y.mean() - y.std(), s=f'-std: {y.mean() - y.std():.2f}', color='g')
    ...
    def num_of_transactions_per_target(
        ...
        if subplots:
            assert (layout[0] * layout[1]) >= (len(self.df[target].unique()) if k == -1 else k), f'The layout is not enough to display all {target}'
        ...
        if k != -1:
            ...
            topk_target = self.df[
                np.in1d(self.df[target], topk_target_idx)
            ]

            target_cnt = topk_target.groupby(
                self.df[self.DATE_COL].dt.to_period(interval)
                ...
                if value_label:
                    for i in range(layout[0]):
                        for j in range(layout[1]):
                            _ = ax[i, j].bar_label(ax[i, j].containers[0], padding=3, fmt='%d')
                if draw_mean_std:
                    for i in range(layout[0]):
                        for j in range(layout[1]):
                            y = target_cnt.iloc[:, j]
                            _ = ax[i, j].axhline(y.mean(), color='b', linestyle='-')
                            _ = ax[i, j].axhline(y.mean() + y.std(), color='r', linestyle='--')
                            _ = ax[i, j].axhline(y.mean() - y.std(), color='g', linestyle='--')
                            _ = ax[i, j].text(x=0, y=y.mean(), s=f'mean: {y.mean():.2f}', color='b')
                            _ = ax[i, j].text(x=0, y=y.mean() + y.std(), s=f'+std: {y.mean() + y.std():.2f}', color='r')
                            _ = ax[i, j].text(x=0, y=y.mean() - y.std(), s=f'-std: {y.mean() - y.std():.2f}', color='g')
...
c = Charter.from_file('datasets/encoded-1.feather')
